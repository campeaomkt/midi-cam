...
export const BottomControls: React.FC<BottomControlsProps> = ({
  ...
  return (
    <div id="bottom-camera-controls" className="w-full flex flex-col items-center gap-2 pb-6 px-4 z-30 select-none">
      ...
      <div className="flex items-center gap-2 bg-black/45 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 shadow-lg text-xs font-bold">
        {zoomLevels.map((lvl) => {
      ...
      <div className="w-full flex items-center justify-between max-w-sm pt-2">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="w-12 h-12 rounded-xl bg-zinc-900 border-2 border-white/20 overflow-hidden flex items-center justify-center transition active:scale-95 shadow-lg group"
          ...
          >
            {lastThumbnailUrl ? (
              ...
              <Film className="w-5 h-5 text-zinc-400 group-hover:text-white" />
            )}
          </button>
          ...
          <button
            ...
            className="w-10 h-10 rounded-full bg-black/50 border border-white/15 flex items-center justify-center text-zinc-300 hover:text-amber-400 transition active:scale-95"
        ...
        <div className="flex flex-col items-center">
          {isRecording && (
            <div className="flex items-center gap-1.5 mb-2 bg-rose-600/90 text-white font-mono text-xs font-bold px-2.5 py-0.5 rounded-full shadow-lg animate-pulse">
          ...
          <button
            ...
            className={`relative flex items-center justify-center rounded-full transition-transform active:scale-90 cursor-pointer ${
              isRecording
                ? 'w-20 h-20 p-2 border-4 border-rose-500 bg-black/40'
                : 'w-20 h-20 p-1.5 border-4 border-white bg-transparent hover:border-zinc-200'
            }`}
            title={isRecording ? 'Parar Gravação' : 'Iniciar Gravação'}
          >
            {isRecording ? (
              <div className="w-7 h-7 rounded-md bg-rose-600 shadow-[0_0_15px_rgba(225,29,72,0.9)] transition-all animate-pulse" />
            ) : (
              <div className="w-full h-full rounded-full bg-rose-600 hover:bg-rose-500 shadow-[0_0_20px_rgba(225,29,72,0.8)] transition-all flex items-center justify-center">
                <Disc className="w-6 h-6 text-white/40 opacity-75" />
              </div>
            )}
          </button>
        </div>
        ...
        <div className="w-12 flex justify-end">
          <button
            ...
            className="w-12 h-12 rounded-full bg-black/50 border border-white/20 flex items-center justify-center text-white hover:bg-black/70 transition active:scale-90 shadow-lg"
            title="Inverter Câmera (Frontal / Traseira)"
          >
            <RefreshCw className="w-5 h-5 transition-transform duration-300 active:rotate-180" />
          </button>
        </div>
      </div>
    </div>
