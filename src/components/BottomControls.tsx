      ...
      <div className="w-full flex items-center justify-between max-w-sm md:max-w-xl lg:max-w-2xl pt-2 px-2">
        ...
        <div className="flex flex-col items-center">
          {isRecording ? (
            ...
            <button
              type="button"
              id="btn-cycle-audio-source"
              onClick={() => {
                const current = cameraSettings.audioRecordSource || (cameraSettings.micEnabled ? 'keyboard-and-mic' : 'keyboard-only');
                if (current === 'keyboard-and-mic') {
                  onUpdateCamera({ audioRecordSource: 'keyboard-only', micEnabled: false });
                } else if (current === 'keyboard-only') {
                  onUpdateCamera({ audioRecordSource: 'mic-only', micEnabled: true });
                } else {
                  onUpdateCamera({ audioRecordSource: 'keyboard-and-mic', micEnabled: true });
                }
              }}
              className="flex items-center gap-1.5 mb-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-black/60 hover:bg-black/80 backdrop-blur-md border border-white/15 transition active:scale-95 cursor-pointer shadow-md text-zinc-200"
              title="Toque para alternar o áudio gravado no vídeo"
            >
              {cameraSettings.audioRecordSource === 'keyboard-only' || (!cameraSettings.micEnabled && cameraSettings.audioRecordSource !== 'mic-only') ? (
                <>
                  <span className="text-amber-400">🎹</span>
                  <span className="text-zinc-200">Áudio: <strong className="text-amber-300">Som SF2 Puro</strong></span>
                </>
              ) : cameraSettings.audioRecordSource === 'mic-only' ? (
                <>
                  <span className="text-emerald-400">🗣️</span>
                  <span className="text-zinc-200">Áudio: <strong className="text-emerald-300">Apenas Mic</strong></span>
                </>
              ) : (
                <>
                  <span className="text-amber-400">🎹</span>
                  ...
                  <span className="text-emerald-400">🎙️</span>
                  <span className="text-zinc-200">Áudio: <strong className="text-amber-300">SF2 + Mic</strong></span>
                </>
              )}
            </button>
          )}
